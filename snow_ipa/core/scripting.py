...
from snow_ipa.services import gdrive
...
class ScriptManager:
    ...
    def update_config(self, new_config: dict | None = None) -> dict:
        """
        Updates the configuration dictionary with values from command-line arguments.
        """
    ...
    def fill_from_files(self) -> None:
        """
        Reads configuration values from files and updates the configuration dictionary.
        """
...
class ExportManager:
    ...
    modis_distinct_months: list = []
    gee_saved_assets: list = []
    gee_saved_assets_months: list = []
    gdrive_saved_assets: list = []
    gdrive_saved_assets_months: list = []

    # Export Plan
    export_plan: dict = {"planned": [], "excluded": {}}
    image_prefix: str = ""
    export_to_gee: bool = False
    gee_assets_path: str = ""
    gdrive_assets_path: str = ""
    export_to_gdrive: bool = False
    months_to_save: list = []  # Starting export plan
    assets_excluded: dict = (
        {}
    )  # Exclusion details. Can include duplicates if the image is being saved to both GEE and GDrive
    gee_assets_to_save: list = []
    gdrive_assets_to_save: list = []

    # Export Results
    export_tasks: ExportList = ExportList()

    def __init__(
        self,
        export_to_gee: bool,
        export_to_gdrive: bool,
        gee_asset_path: str = "",
        gdrive_asset_path: str = "",
        months_to_save: list = [],
        image_prefix: str = "",
    ) -> None:
        self.export_to_gee = export_to_gee
        self.gee_assets_path = gee_asset_path
        self.export_to_gdrive = export_to_gdrive
        self.gdrive_assets_path = gdrive_asset_path
        self.image_prefix = image_prefix

        # If no explicit request, save last month
        if not months_to_save:
            prev_month = dates.prev_month_last_date().strftime("%Y-%m")
